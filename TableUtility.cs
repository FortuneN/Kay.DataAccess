...
namespace Kay.DataAccess
{
	...
	public class TableUtility
	{
		...
		public TableUtility(Type itemType, TableAttribute tableAttribute, PropertyInfo primaryPropertyInfo) : this(null, itemType, tableAttribute, primaryPropertyInfo) { }
		public TableUtility(DataContext dataContext, Type itemType, TableAttribute tableAttribute, PropertyInfo primaryPropertyInfo)
		{
		...
		private static Dictionary<string, TableUtility> cache = new Dictionary<string, TableUtility>();

		public static TableUtility GetByType(Type itemType)
		{
			TableUtility result = null;
			...
			if (!cache.ContainsKey(key) || (result = cache[key]) == null)
			{
				...
				foreach (PropertyInfo property in itemType.GetProperties())
				{
					...
					{
						if (column.IsPrimaryKey) result = new TableUtility(itemType, table, property);
